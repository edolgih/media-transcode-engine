...
public sealed class GpuEncodeRoute : ITranscodeRoute
{
    private readonly ITranscodeExecutionPipeline _pipeline;

    public GpuEncodeRoute(ITranscodeExecutionPipeline pipeline)
    {
        _pipeline = pipeline;
    }
    ...
    {
        return _pipeline.ProcessByKey(BuildStrategyKey(request), request);
    }

    public string ProcessWithProbeResult(TranscodeRequest request, ProbeResult? probe)
    {
        return _pipeline.ProcessByKeyWithProbeResult(BuildStrategyKey(request), request, probe);
    }

    public string ProcessWithProbeJson(TranscodeRequest request, string? probeJson)
    {
        return _pipeline.ProcessByKeyWithProbeJson(BuildStrategyKey(request), request, probeJson);
    }

    private static string BuildStrategyKey(TranscodeRequest request)
    {
        ArgumentNullException.ThrowIfNull(request);
        var codecToken = request.TargetVideoCodec.Trim().ToLowerInvariant();
        return CodecExecutionKeys.BuildGpuEncodeKey(codecToken);
    }
}
